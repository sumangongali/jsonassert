...
public class DefaultComparator extends AbstractComparator {
    ...
    public void compareJSON(String prefix, JSONObject expected, JSONObject actual, JSONCompareResult result, String maskWord, String splitChars)
            throws JSONException {
        // Check that actual contains all the expected values
        checkJsonObjectKeysExpectedInActual(prefix, expected, actual, result, maskWord, splitChars);
    ...
    public void compareJSONArray(String prefix, JSONArray expected, JSONArray actual, JSONCompareResult result, String maskWord, String splitChars)
            throws JSONException {
        if (expected.length() != actual.length()) {
            result.fail(prefix + "[]: Expected " + expected.length() + " values but got " + actual.length());
            return;
        ...
        if (mode.hasStrictOrder()) {
            compareJSONArrayWithStrictOrder(prefix, expected, actual, result,maskWord,splitChars);
        } else if (allSimpleValues(expected)) {
            compareJSONArrayOfSimpleValues(prefix, expected, actual, result,maskWord,splitChars);
        } else if (allJSONObjects(expected)) {
            compareJSONArrayOfJsonObjects(prefix, expected, actual, result,maskWord,splitChars);
        } else {
            // An expensive last resort
            recursivelyCompareJSONArray(prefix, expected, actual, result,maskWord,splitChars);
        }
    }

	@Override
	public void compareValues(String prefix, Object expectedValue,
			Object actualValue, JSONCompareResult result, String maskWord,
			String splitChars) throws JSONException {
		//compareValues(prefix, expectedValue, actualValue, result, ".#", " \t");
