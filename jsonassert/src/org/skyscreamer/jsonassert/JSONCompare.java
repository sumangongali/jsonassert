...
import org.skyscreamer.jsonassert.comparator.MaskComparator;
...
public final class JSONCompare {
    ...
    private static JSONComparator getComparatorForMode(JSONCompareMode mode) {
       // return new DefaultComparator(mode);
    	return new MaskComparator(mode);
    }
    ...
    public static JSONCompareResult compareJSON(String expectedStr, String actualStr, JSONComparator comparator, String maskWord, String splitChars)
            throws JSONException {
        Object expected = JSONParser.parseJSON(expectedStr);
        Object actual = JSONParser.parseJSON(actualStr);
        if ((expected instanceof JSONObject) && (actual instanceof JSONObject)) {
        	//JSONObject.
            return compareJSON((JSONObject) expected, (JSONObject) actual, comparator, maskWord, splitChars);
        }
        else if ((expected instanceof JSONArray) && (actual instanceof JSONArray)) {
            return compareJSON((JSONArray)expected, (JSONArray)actual, comparator, maskWord, splitChars);
        }
        else if (expected instanceof JSONObject) {
            return new JSONCompareResult().fail("", expected, actual);
        }
    ...
    public static JSONCompareResult compareJSON(JSONObject expected, JSONObject actual, JSONComparator comparator, String maskWord, String splitChrs)
            throws JSONException {
        return comparator.compareJSON(expected, actual, maskWord, splitChrs);
    }
    ...
    public static JSONCompareResult compareJSON(JSONArray expected, JSONArray actual, JSONComparator comparator, String maskWord, String splitChars)
            throws JSONException {
        return comparator.compareJSON(expected, actual,maskWord, splitChars);
    }
    ...
    public static JSONCompareResult compareJSON(String expectedStr, String actualStr, JSONCompareMode mode, String maskWord, String splitChars)
            throws JSONException {
        return compareJSON(expectedStr, actualStr, getComparatorForMode(mode), maskWord, splitChars);
    }
